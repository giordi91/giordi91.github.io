...
parallel transport is the natural twist the frame has, following down the curve; You can imagine it as 
...
basically we for the ith iteration we use the resulting upvector from i-1th iteration. 
...
To note that we added the atttribute **[ExecuteInEditMode]**, this allow the
script to be executed even when not in play mode, making our life much easier.

I am using the new great 
Unity.Mathematics library **MISSING LINK**
...
to not introduce unwanted negatie scales.
...
After performing the cross product we finally have 3 axis, we are ready 
...
<a href="https://en.wikipedia.org/wiki/Rotation_matrix#Rotation_matrix_from_axis_and_angle" target="_blank">a rotation matrix from an axis and an angle</a>  
...
As you might have noticed there is one parameter that is really important, 
the initial up vector, is up to you to have or find an starting up 
vector that makes sense. 

The system has several limitations that can make it fail, 
if any up vector is parallel to the aim (aka 90 degrees bend) the cross 
product will fail, or if you have a bend that is bigger than 90 degrees 
...
runtime.
In realty that is not too much of a concern, most of the time the curves
that are used (either descrete or mathematically defined) are smooth and well
behaved.
...
with a consistante orientation, that is not all, once you have a stable frame
