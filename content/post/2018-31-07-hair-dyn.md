...
The first and simples of those is the global constraints, a constraints that happens completely in
...
Here an extract from the AMD paper:
...
Is a bit hard to see, but in purple, you can see the original hair being deformed rigidly,
...
The hair bent quite uniformly from the original frame, meaning each segment roughly bent the same amount,
but if you look on how much each particle has been corrected you will see that the more down the curve
we go, the more correction is applied, this is because, the rotation on the segments is sort of
...
this can be done with the **Parallel transport** method you can find in my previous {{<target-blank "article" "post/2018-31-07-parallel-transport">}}.

![localC](../images/02_hairDyn/localConstraintDraw.jpg)

Once you have those generated local frames you are going to perform the constraint in this space
...
into the local frame, involving an inverse matrix or, you can have the original hair, which has been converted in local space, being converted in back in world space using the new computed local frame.
...
to do it is with quaternions, that is how TressFX is doing it, in my first iteration I went with matrices
so that I did not have to implement the quaternion math myself. With this new knowledge, if we grab the local delta of $P\_4$ from the original hair (black hair) and multiply it by the local frame of the red
hair we get something like this:
...
the global constraint. This is the same implementation I came up with, and worked quite well for
short segment of hair, I was experimenting with 5 particles, as soon as you have more it starts to
ad a lot of oscillation, creating a wave ripple down the hair. If you think of it, we start straighting
up the hair from the root up, creating a sort of whip effect, that is where I got stuck in my implementation, the researcher corrected this by applying the correction not only at the end of the segment
but also at the start of it, basically achieve a rotation from the center of the segment instead applying the rotation at the base of the segment. The final result with only local constraint and edge constraint is the following:

![local](../images/02_hairDyn/local.gif)

As you can see the effect is quite different from the global constraint, the local one fights to keep
the shape without fighting the overall simulation, the one in the image is with a fairly high stiffness.
The artist will end up playing with how many and how strong the shape constraint are to achieve the
desired effect. To note that the same issue we had with edge constraints appears here, we are modifying
two particles at the same time, which will make us applying the constraint multiple times.
